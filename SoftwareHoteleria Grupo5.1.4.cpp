...
void menuFinanzas(const inventario objetos[], int cantidadArt, const vector<Habitacion>& habitaciones, const empleados empleadosHotel[], int cantidadEmpleados) {
    ...
    cout << left << setw(30) << "(Ingreso) Ventas habitacion: S/." 
         << setw(30) << "(Egreso) Costo inventario: S/." 
         << setw(30) << "(Egreso) Costo personal: S/."
         << "Balance total: S/." << endl;
...
void modificarCant(inventario objeto[], int cantidadArt) {
    ...
    for (int i = 0; i < cantidadArt; ++i) {
        if (strcmp(objeto[i].nombre, busqueda) == 0) {
            ...
            objeto[i].precioFinal = objeto[i].cantidad * objeto[i].precio;
            cin.ignore();
            encontrado = true;
            break;
        }
...
void mostrarInventario(inventario objetos[], int dimension) {
    ...
    for (int i = 0; i < dimension; ++i) {
        cout << left << setw(30) << objetos[i].nombre
             ...
             << setw(15) << objetos[i].precioFinal << endl;
    }
    getch();
}

void menuInventario(inventario objetos[], int& cantidadArt, int& ultimoArticulo, int& dimension) {
...
void menuHabitacion(vector<Habitacion>& habitaciones) {
    int opcion;
    do {
        opcion = mostrarMenu();
...
void eliminarEmpleado(empleados empleado[], int& cantidadEmpleados) {
    ...
    if (encontrado) {
        for (int i = aux; i < cantidadEmpleados - 1; ++i) {
            strcpy(empleado[i].nombre, empleado[i + 1].nombre);
...
void menuEmpleados(empleados empleadosHotel[], int& cantidadEmpleados) {
    int opcion;
    do {
        guardarEmpleados(empleadosHotel, cantidadEmpleados);
        system("cls");
...
int main() {
    ...
    do {
        ...
        switch (menu) {
            case 1:
                menuHabitacion(habitaciones);
                break;
            case 2:
                menuInventario(objetos, cantidadArt, ultimoArticulo, dimension);
                break;
            case 3:
                menuEmpleados(empleadosHotel, cantidadEmpleados);
                break;
            case 4:
